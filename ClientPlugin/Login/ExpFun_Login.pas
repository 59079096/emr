...
begin
  ...
  AIPlugin.Comment := '完成系统登录';  // 插件说明
  AIPlugin.ID := PLUGIN_LOGIN; // 插件GUID，即唯一标识
  AIPlugin.Name := '系统登录';  // 插件功能或业务名称
  AIPlugin.Version := '1.0.0';  // 插件版本号
  //
  with AIPlugin.RegFunction(FUN_BLLFORMSHOW, '登录') do
    ShowEntrance := False;  // 在界面显示调用入口
end;
